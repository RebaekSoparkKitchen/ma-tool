...
class SqlWriter(MA):
    ...
    def __init__(self, campaignId: int):
        ...
        self.dbAddress = config['data_location']['Database']
    ...
    def insertIntoClick(self) -> None:
        ...
        sqlList = []
        for item in click:
            data = (item['smc_campaign_id'], item['Content Link Name'], item['Clicks'], item['Link Alias'], item['if_main_click'], item['creation_time'], self.username)
