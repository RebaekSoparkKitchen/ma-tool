...
def report_work():
    ...
    sql = f"SELECT {','.join(cols)} FROM Request " \
          f"WHERE DATE(report_date) = DATE('now', 'localtime') " \
          f"ORDER BY id"
    title = '今日需发送报告'
    return data_producer(title=title, cols=cols, sql=sql)
...
def campaign_id_work():
    ...
    sql = f"SELECT {','.join(cols)} FROM Request " \
          "WHERE smc_campaign_id IS NULL AND DATE(blast_date) BETWEEN DATE('2020-01-01') AND DATE('now', 'localtime') " \
