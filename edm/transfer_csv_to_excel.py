...
class ExportedData:
    ...
    def __init__(self, path):
        ...
        self.df = pd.read_csv(open((path), 'r', encoding='utf-8'),sep=';')
        self.path = path
    ...
    def clean_data(self):
        ...
        for i in ['Acc Phone Number','CP Phone Number', 'CP Mobile Phone Number','Opt-out Phone','Opt-in Phone']:
            try:
                df[i] = df[i].apply(lambda x: str(int(x)) if pd.notnull(x) else x)  #尝试先对他们整数化，如若不行，eg:+85228880888|+85263680340，则直接str()
            except ValueError:
                df[i] = df[i].apply(lambda x: str(x) if pd.notnull(x) else x)
...
if __name__ == "__main__":
    e = Highlight(r'C:/Users/C5293427/Desktop/Data_exported/成品/CN_智能ERP_open.xlsx')
    e.add_color(r'C:/Users/C5293427/Desktop/Data_exported/csv_file/CN_智能ERP_click_email.csv')
